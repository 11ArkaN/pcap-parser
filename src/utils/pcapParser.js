...
export async function parsePcap(buffer, options = {}) {
    const result = await parsePcapDetailed(buffer, options);
    return result.connections;
}
export async function parsePcapDetailed(buffer, options = {}) {
...
function parsePcapLegacy(data, maxConnections) {
    const connections = [];
...
function parsePcapNg(data, maxConnections) {
    const connections = [];
    let offset = 0;
    let truncated = false;
    ...
    while (offset < data.length) {
        ...
        if (blockLen === 0 || offset + blockLen > data.length)
            break;
...
function buildConnection(src, dst, protocol, srcPort, dstPort, type) {
    return {
        src,
        dst,
        protocol,
